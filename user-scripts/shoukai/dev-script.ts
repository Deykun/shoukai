...
userScriptApp.get("/server.user-script.js", (req, res) => {
  const devScript = fs.readFileSync(
    "user-scripts/shoukai/dev.user-script.js",
    "utf-8"
  );
  ...
  console.log(
    chalk.blue(`Version ${version} of dev script has been downloaded!`)
  );
  console.log("");
...
userScriptApp.get("/dev.user-script.js", (req, res) => {
  ...
  const devScript = fs.readFileSync(
    "public/user-script/shoukai.user.js",
    "utf-8"
  );
  ...
  res.send(
    devScript.replace(
      ...
      `const shoukaiUrl = 'http://localhost:3001/shoukai/';`
    )
  );
});
